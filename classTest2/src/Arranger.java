...
public class Arranger {
     ...
     *
     * @param args 行列式数据文件
     * @throws IOException 当读取用户输入错误时抛出
     */
     ...
     *
     * @param arrangerFileName 行列式的数据文件
     * @return 包含行列式数据的二维数组
     * @throws IOException
     */
    private static double[][] readFile(String arrangerFileName)
            throws IOException {
        double[][] newArrangerMatrix;
        StringTokenizer stringTokenizer;
        BufferedReader bufferedReader = new BufferedReader(new FileReader(arrangerFileName));
        String line = bufferedReader.readLine();
        stringTokenizer = new StringTokenizer(line, ",");
        int n = stringTokenizer.countTokens();
        newArrangerMatrix = new double[n][n];
        int r = 0;
        while (line != null || r < n) {
            stringTokenizer = new StringTokenizer(line, ",");
            for (int c = 0; c < n; c++) {
                newArrangerMatrix[r][c] = Double.parseDouble(stringTokenizer.nextToken());
            }
            r++;
    ...
    private void saveMatrix(String fileName) throws IOException {
        ...
        for (int r = 0; r < arrangerMatrix.length; r++) {
            for (int c = 0; c < arrangerMatrix[r].length; ++c) {
                printWriter.print(arrangerMatrix[r][c]);
                if (c != (arrangerMatrix[r].length - 1)) {
                    printWriter.print(",");
                }
            }
            printWriter.println();
